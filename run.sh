...
mv ../annotate_cogs.py bin/annotate_cogs.py
mv ../read_members_file.py bin/read_members_file.py
...
zcat $bacteria | bin/read_members_file.py -min_occurence 99 > data/cogs_bacteria_o99.txt
zcat $archaea | bin/read_members_file.py -min_occurence 99 > data/cogs_archaea_o99.txt
zcat $eukaryota | bin/read_members_file.py -min_occurence 99 > data/cogs_eukaryota_o99.txt


### 2. Which bacterial genes occur in at least 50% of all bacterial genomes, and in at least 99% thereof as single-copy?

zcat $bacteria | ./bin/read_members_file.py -min_occurence 50 -min_uniqueness 99 > data/cogs_bacteria_o50_u99.txt

# How many of these OGs were also identified as universal bacterial OGs (previous question)?
comm -12 data/cogs_bacteria_o99.txt data/cogs_bacteria_o50_u99.txt


### 3. Identify all OGs that occur as single-copy in at least 97% of all archaea

zcat $archaea | ./bin/read_members_file.py -min_occurence_as_singlecopy 97 > data/cogs_archaea_os99.txt
...
zcat $archaea_fct | ./bin/annotate_cogs.py data/cogs_archaea_os99.txt
